...
# -------------------------
SERVICE_ACCOUNT_FILE = "bp-simulator-key.json"  # ← Your JSON key file here
SCOPES = [
...
try:
    ...
    sheet = gc.open("BP Simulator Log").sheet1
    gs_ready = True
...
with col2:
    ...
    location = st.selectbox("Current Location", [
        "Geneva", "Zurich", "Dubai", "London", "Singapore", "Hong Kong", "New York", "Miami"
    ])
with col3:
...
total_row = pd.DataFrame([[np.nan, proj_df["NNM (M)"].sum(), np.nan, proj_df["Revenue"].sum()]], columns=proj_df.columns)
...
if not prospects.empty:
    ...
    total_row = pd.DataFrame([["TOTAL", "", prospects[numeric_cols[0]].sum(),
                               prospects[numeric_cols[1]].sum(), prospects[numeric_cols[2]].sum()]],
...
# -------------------------
if gs_ready and candidate_name:
    try:
        sheet.append_row([candidate_name, candidate_email, current_role, current_employer,
                          years_exp, location, market, current_book, last_bonus, total_nnm, avg_roa])
        st.info("✅ Candidate logged to Google Sheets.")
    except Exception as e:
